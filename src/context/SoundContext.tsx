...
export const SoundProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Check if the browser allows autoplay right away (e.g. Chrome Media Engagement Index)
    if (localStorage.getItem(PREFERENCE_KEY) === 'enabled') {
      ...
      if (p !== undefined) {
        p.then(() => {
          ...
          // Standard browser autoplay policy: will be unlocked on user gesture
        });
      }
    }
  ...
  // SUBSEQUENT VISITS:
  // If preference is 'enabled' but AudioContext is still suspended by the browser,
  // synchronously unlock and play on first genuine user interaction
  useEffect(() => {
    if (soundPreference !== 'enabled' || isUnlocked) return;

    const handleSubsequentUnlock = () => {
      const ctx = getAudioContext();
      if (ctx && ctx.state === 'suspended') {
        ctx.resume().catch(() => {});
      }
    ...
    const cleanup = () => {
      window.removeEventListener('click', handleSubsequentUnlock, { capture: true });
      window.removeEventListener('touchstart', handleSubsequentUnlock, { capture: true });
      window.removeEventListener('keydown', handleSubsequentUnlock, { capture: true });
    };

    window.addEventListener('click', handleSubsequentUnlock, { capture: true, once: true });
    window.addEventListener('touchstart', handleSubsequentUnlock, { capture: true, once: true });
    window.addEventListener('keydown', handleSubsequentUnlock, { capture: true, once: true });

    return cleanup;
  }, [soundPreference, isUnlocked, getAudioContext]);
