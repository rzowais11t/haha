...
export default function Process() {
  const cards = [
    ...
    {
      title: 'Optimization',
      text: 'We remain a strategic technical partner, continuously improving your systems as your business evolves.',
      icon: (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" className="w-[72px] h-[72px] stroke-[1.2]">
          <circle cx="8.5" cy="7.5" r="4"></circle>
          <path d="M4.5 21v-2a4 4 0 0 1 4-4h3"></path>
          <path d="M18 22s4-2 4-5V13.5l-4-1.5-4 1.5V17c0 3 4 5 4 5z"></path>
        </svg>
      )
    }
  ];
  ...
  return (
    <SectionWrapper>
      ...
      <div className="grid grid-cols-1 min-[900px]:grid-cols-3 mt-12 bg-card-bg border-2 border-border rounded-lg overflow-hidden reveal">
        {cards.map((card, idx) => (
          <div
            key={idx}
            className="p-[4rem_3rem] relative flex flex-col transition-colors duration-300 hover:bg-white/5 border-b-2 border-border min-[900px]:border-r-2 max-[900px]:border-r-0 max-[900px]:last:border-b-0 min-[900px]:[&:nth-child(3n)]:border-r-0 min-[900px]:[&:nth-child(n+4)]:border-b-0"
          >
            <div className="w-full flex justify-center mb-10 text-text">
              {card.icon}
            </div>
            <h3 className="font-sans text-[1.3rem] font-semibold mb-4 text-text tracking-[-0.02em]">{card.title}</h3>
            <p className="text-[0.95rem] text-muted leading-[1.6]">{card.text}</p>
          </div>
        ))}
      </div>
    </SectionWrapper>
